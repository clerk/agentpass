...
describe('Authority Handler', () => {
  ...
  describe('POST /agentpass-authority/validate', () => {
    it('rejects the wrong service without consuming the AgentPass', async () => {
      ...
      const validateHandler = createAuthorityHandler(
        {
          ...testConfig,
      ...
      const wrongReq = new Request('https://authority.example.com/agentpass-authority/validate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${await signServiceAssertion(wrongService.privateJwk, 'https://evil.example.com')}`,
        },
        body: JSON.stringify({ agentpass: { value: 'ap_test' } }),
      });

      const wrongRes = await validateHandler(wrongReq);
      expect(wrongRes.status).toBe(403);

      const rightReq = new Request('https://authority.example.com/agentpass-authority/validate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      const rightRes = await validateHandler(rightReq);
...
async function signServiceAssertion(privateJwk: JsonWebKey, serviceOrigin: string): Promise<string> {
