# 3.6.4. GET {agentpass_configuration_url}/requests/{id}/events (Optional)
...
Clients MAY consume SSE for lower latency, but MUST tolerate disconnects and resume via `GET {agentpass_configuration_url}/requests/{id}`.
