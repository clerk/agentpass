...
export function generateAgentPassValue(): string {
  const bytes = new Uint8Array(32);
  crypto.getRandomValues(bytes);
  return 'ap_' + Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join('');
}
...
export async function fetchJwks(jwksUri: string): Promise<JsonWebKey[]> {
  ...
  if (!response.ok) throw new Error(`Failed to fetch JWKS from ${jwksUri}: ${response.status}`);
