...
```json
{
  ...
  "endpoints": {
    "requests": "https://agentpass.example.com/requests",
    "request_status": "https://agentpass.example.com/requests/{id}",
    "events": "https://agentpass.example.com/requests/{id}/events"
  },
  "approval": {
    "modes": ["poll", "sse"],
...
- `endpoints.requests` (required): `POST` request-creation endpoint.
- `endpoints.request_status` (required): request-status lookup URL template (includes `{id}`).
- `endpoints.events` (optional): SSE events URL template (includes `{id}`).
