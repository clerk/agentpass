...
// ../../packages/service/dist/index.js
async function importSigningKey(jwk) {
  const alg = jwk.kty === "EC" ? { name: "ECDSA", namedCurve: jwk.crv || "P-256" } : jwk.kty === "RSA" ? { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" } : (() => {
...
__name(importVerifyKey, "importVerifyKey");
...
__name(getAlg, "getAlg");
function getSignParams(key) {
  if (key.algorithm.name === "ECDSA") return { name: "ECDSA", hash: "SHA-256" };
  return { name: "RSASSA-PKCS1-v1_5" };
}
__name(getSignParams, "getSignParams");
...
__name(base64urlDecodeBuffer, "base64urlDecodeBuffer");
function createServiceHandler(config) {
  const basePath = (config.basePath || "").replace(/\/$/, "");
  return /* @__PURE__ */ __name(async function handleRequest(request) {
    const url = new URL(request.url);
    const path = url.pathname;
    if (request.method === "GET" && path === `${basePath}/.well-known/agentpass-service.json`) {
      return handleConfiguration();
    }
    if (request.method === "GET" && path === `${basePath}/.well-known/jwks.json`) {
      return handleJwks();
    }
  ...
  function handleConfiguration() {
    const configDoc = {
      ...
      jwks_uri: config.jwksUri || `${config.origin}${basePath}/.well-known/jwks.json`,
      trust: {
  ...
  __name(handleJwks, "handleJwks");
  async function handleResolveAuthorities(request) {
    let body;
    try {
    ...
    if (config.onResolveAuthority) {
      try {
        ...
        return errorResponse(500, "resolution_error", e.message);
      }
    }
      ...
      try {
        ...
        return json({
          enterprise_authority: {
            ...
            agentpass_configuration_url: dnsResult
          }
        });
      } catch {
    ...
    return errorResponse(404, "no_authority", "No authoritative provider and no trusted federated options");
  }
  __name(handleResolveAuthorities, "handleResolveAuthorities");
  async function handleAvailableScopes(request) {
    ...
    try {
      ...
      await verifyAuthorityAssertion(authHeader.slice(7), payload.iss);
    } catch (e) {
      return errorResponse(401, "invalid_assertion", `Assertion verification failed: ${e.message}`);
    }
    let body;
    try {
      body = await request.json();
    ...
    try {
      ...
      return errorResponse(500, "scope_error", e.message);
    }
  }
  __name(handleAvailableScopes, "handleAvailableScopes");
  async function handleRedeemBrowserSession(request) {
    let body;
    try {
      body = await request.json();
    } catch {
    ...
    try {
      ...
      return errorResponse(422, "validation_failed", e.message);
    }
    let grantedScope = validation.scope;
    if (body.requested_scope) {
      grantedScope = validation.scope.includes("*") ? body.requested_scope : body.requested_scope.filter((s) => validation.scope.includes(s));
    ...
    try {
      ...
      return errorResponse(500, "redemption_error", e.message);
    }
  }
  __name(handleRedeemBrowserSession, "handleRedeemBrowserSession");
  async function handleRedeemBearerToken(request) {
    let body;
    try {
      body = await request.json();
    } catch {
  ...
  __name(handleRedeemBearerToken, "handleRedeemBearerToken");
  ...
  __name(isTrustedAuthority, "isTrustedAuthority");
  async function validateAgentPassWithAuthority(authority, agentpassValue) {
    let configUrl;
    if (config.trust.serviceAuthority?.authority === authority) {
      configUrl = config.trust.serviceAuthority.agentpass_configuration_url;
    }
    if (!configUrl) {
      const fed = config.trust.trustedFederatedAuthorities?.find((a) => a.authority === authority);
      if (fed) configUrl = fed.agentpass_configuration_url;
    }
    if (!configUrl) throw new Error("Authority not found in trust configuration");
    const authConfig = await fetchAuthorityConfig(configUrl);
  ...
  __name(validateAgentPassWithAuthority, "validateAgentPassWithAuthority");
  async function verifyAuthorityAssertion(jwt, authorityId) {
    let configUrl;
    ...
    if (configUrl) {
      ...
      jwksUri = `${authorityId}/.well-known/jwks.json`;
    }
  ...
  __name(verifyAuthorityAssertion, "verifyAuthorityAssertion");
  async function discoverEnterprise(emailDomain) {
    try {
  ...
  __name(discoverEnterprise, "discoverEnterprise");
}
__name(createServiceHandler, "createServiceHandler");
async function fetchAuthorityConfig(configUrl) {
  const response = await fetch(configUrl);
  if (!response.ok) throw new Error(`Failed to fetch authority config: ${response.status}`);
  return response.json();
}
__name(fetchAuthorityConfig, "fetchAuthorityConfig");
async function defaultDnsResolver(name, type) {
...
__name(defaultDnsResolver, "defaultDnsResolver");
...
// worker/agentpass/scopes.ts
var onScopeDiscovery = /* @__PURE__ */ __name(async () => [
  { name: "todos:read", description: "Read todos" },
  { name: "todos:write", description: "Create, update, and delete todos" }
], "onScopeDiscovery");

// worker/agentpass/redemption.ts
var apiTokens = /* @__PURE__ */ new Map();
...
function createRedeemBrowserSession(serviceOrigin) {
  return async (params) => {
...
__name(createRedeemBrowserSession, "createRedeemBrowserSession");
function createRedeemBearerToken() {
  return async (params) => {
...
__name(createRedeemBearerToken, "createRedeemBearerToken");

// worker/agentpass/index.ts
function createAgentPassHandler(c) {
  const env = c.env;
  const signingKey = JSON.parse(env.SIGNING_KEY);
  return createServiceHandler({
    origin: env.SERVICE_ORIGIN,
    basePath: "/agentpass-service",
    name: "AgentPass Todo Service",
    jwksUri: `${env.SERVICE_ORIGIN}/agentpass-service/.well-known/jwks.json`,
    trust: {
      trustedFederatedAuthorities: [
        {
          authority: env.AUTHORITY_URL,
          agentpass_configuration_url: env.AUTHORITY_CONFIG_URL
        }
      ]
    },
    signingKey,
    ...
    dnsResolver: /* @__PURE__ */ __name(async () => [], "dnsResolver")
  });
}
__name(createAgentPassHandler, "createAgentPassHandler");
...
app.patch("/api/todos/:id", async (c) => {
  ...
  if (body.title !== void 0) todo.title = body.title;
  if (body.completed !== void 0) todo.completed = body.completed;
  return c.json({ todo });
});
